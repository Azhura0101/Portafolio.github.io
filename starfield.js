...
export default class Starfield {
  ...
  buildStars() {
    const count = this.mobile ? 500 : 1500;
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (rs < 0.5) size[i] = 0.2 + Math.random() * 0.4;
      else if (rs < 0.8) size[i] = 0.7 + Math.random() * 0.6;
      else if (rs < 0.95) size[i] = 1.5 + Math.random() * 1.0;
      else size[i] = 2.5 + Math.random() * 2.5;
    ...
    for (let i = 0; i < count; i++) {
      const b = i * 3;
      angle[i] = Math.atan2(base[b + 2], base[b]);
      radius[i] = Math.sqrt(base[b] * base[b] + base[b + 2] * base[b + 2]);
    }

    this.starAngle = angle;
    this.starRadius = radius;
    this.starZOff = new Float32Array(count);
    this.starBase = base;
    this.starGeom = geom;
    this.starCount = count;

    this.mat = new THREE.ShaderMaterial({
      uniforms: { uWarp: { value: 0 }, uTime: { value: 0 }, uTexture: { value: glowTexture() } },
      vertexShader: `
        ...
        varying vec3 vColor; varying float vAlpha; varying float vDist;

        void main() {
          vec3 p = position;
          float w = uWarp;

          p.xy *= 1.0 + w * aSpeed * 2.5;
          p.z += w * aSpeed * 15.0;

          vec4 mv = modelViewMatrix * vec4(p, 1.0);
          float scale = 300.0 / max(-mv.z, 1.0);
          float sizeMult = 1.0 + w * aSpeed * 3.0;
          gl_PointSize = aSize * scale * sizeMult;
          gl_PointSize = max(gl_PointSize, 0.5);
          gl_Position = projectionMatrix * mv;

          vColor = aColor;
          vDist = abs(position.z);
          vAlpha = 0.06 + (1.0 - smoothstep(0.0, 120.0, vDist)) * 0.7;
          vAlpha = clamp(vAlpha + w * aSpeed * 0.3, 0.0, 1.0);
        }
      `,
      fragmentShader: `
        uniform float uTime; uniform sampler2D uTexture;
        varying vec3 vColor; varying float vAlpha; varying float vDist;

        void main() {
          vec4 tex = texture2D(uTexture, gl_PointCoord);
          if (tex.a < 0.005) discard;
          float twinkle = 0.85 + 0.15 * sin(uTime * (0.6 + vDist * 0.02) + vDist * 0.1);
          float a = tex.a * vAlpha * twinkle;
          vec3 tint = vColor * (1.0 + tex.r * 0.15);
  ...
  animate() {
    ...
    for (let i = 0; i < this.starCount; i++) {
      const b = i * 3;
      const phase = this.starGeom.attributes.aPhase.array[i];
      const s = speedArr[i];

      const r = this.starRadius[i];
      const speedMod = 0.7 + 0.3 * (0.5 + 0.5 * Math.sin(t * 0.11 * s + phase)) * (0.5 + 0.5 * Math.sin(t * 0.19 * s + phase * 2.3));
      this.starAngle[i] += dt * (0.008 / (1 + r * 0.004)) * speedMod;
      const a = this.starAngle[i];
      const rm = 1 + Math.sin(t * 0.06 * s + phase * 1.7) * 0.04;
      const xOrb = r * rm * Math.cos(a);
      const zOrb = r * rm * Math.sin(a);

      const df = 1 - this.warp * 0.8;
      pos[b] = xOrb + Math.sin(t * 0.06 * s + phase) * 0.3 * df;
      pos[b + 1] = base[b + 1] + Math.cos(t * 0.04 * s + phase * 1.3) * 0.25 * df;

      this.starZOff[i] = Math.max(-8, this.starZOff[i] + dw * s * 150);
      pos[b + 2] = zOrb + this.starZOff[i];

      const recycle = (this.warp > 0.05 && pos[b + 2] > 5) || pos[b + 2] < -120;
      if (recycle) {
        const theta = Math.random() * Math.PI * 2;
        const phi = Math.acos(2 * Math.random() - 1);
